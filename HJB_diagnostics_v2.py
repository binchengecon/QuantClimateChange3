...
T_0 = To + min(T_grid)
C_0 = 275.5
# (870 - 580) / 2.13 # total cumulated, as of now, preindustrial with Fo
F_0 = min(F_grid)
...
for tm in range(pers):
    if tm == 0:
        ...
        hist[tm, 0] = max(hist[tm-1, 0] + mu_T(hist[tm-1, :])
                          * dt, To + min(T_grid))
...
plt.ylim(-0.1)
plt.savefig(
    f"./figure/Econ_Climate/Diag2T_C_E_{cearth}_{tauc}.pdf")
