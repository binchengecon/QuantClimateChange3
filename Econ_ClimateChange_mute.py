...
parser.add_argument("--simutime",type=float)

parser.add_argument("--alphamute",type=int)
parser.add_argument("--betamute",type=int)


parser.add_argument("--hXarr",nargs='+',type=float)
parser.add_argument("--Xminarr",nargs='+',type=float, default=(1e-8, 200.0, 1e-8))
...
def model(cearth=0.3916,tauc = 30):
    ...
    def alphaocean(T):
        ...
        elif (args.alphamute==1):
            temp = 0.30*np.ones(T.shape)
    ...
    def fracseaice(T):
        ...
        elif (args.betamute==1):
            temp = 0.10*np.ones(T.shape)
    ...
    # plt.plot(T_grid+To, α_ocean)
    # plt.xlabel("Temperature anomaly")
    # plt.title("$α_{oc}(T)$")
    # plt.savefig("./figure/Econ_Climate/alpha.pdf")
    # plt.close()
    ...
    # plt.xlabel("Temperature anomaly")
    # plt.title("f(T)")
    # plt.savefig("./figure/Econ_Climate/beta.pdf")
...
def simulation(T_grid,C_grid,F_grid,Ca,cearth=0.3916,tauc = 30):
    ...
    plt.ylim(-0.1)
    plt.savefig(f"./figure/Econ_Climate/T_C_E_{cearth}_{tauc}_alpha{args.alphamute}_beta{args.betamute}_max{args.Xmaxarr}.pdf")
